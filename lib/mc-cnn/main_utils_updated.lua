...
function stereo_predict(output_file_bin_2, x_batch, id)
   local vols, vol
...
--            vol = vol:transpose(3, 4):transpose(2, 3):clone()
      ...
      if opt.a == 'predict' then
         ...
         print(('Writing %s, %d x %d x %d x %d'):format(output_file_bin_2, vol:size(1), vol:size(2), vol:size(3), vol:size(4)))
         torch.DiskFile(output_file_bin_2, 'w'):binary():writeFloat(vol:float():storage())
         collectgarbage()
      end
