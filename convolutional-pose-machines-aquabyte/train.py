...
from models.nets import cpm_hand_slim, cpm_body, cpm_body_slim
...
tf.app.flags.DEFINE_string('tfr_data_files',
                           default='./cpm_sample_dataset_512x512.tfrecords',
                           docstring='Training data tfrecords')
# tf.app.flags.DEFINE_string('pretrained_model',
#                            default='cpm_body.pkl',
...
tf.app.flags.DEFINE_integer('gaussian_radius',
                            default=8,
                            docstring='Center map gaussian variance')
...
tf.app.flags.DEFINE_string('saved_model_name',
                           default='_cpm_body_i' + str(FLAGS.input_size) + 'x' + str(
...
tf.app.flags.DEFINE_string('log_dir',
                           default='logs/_cpm_body_i' + str(FLAGS.input_size) + 'x' + str(
                               FLAGS.input_size) + '_o' + str(FLAGS.heatmap_size) + \
                                         'x' + str(FLAGS.heatmap_size) + '_' + str(
                               FLAGS.stages) + 's',
...
tf.app.flags.DEFINE_string('color_channel',
                           default='RGB',
                           docstring='')
tf.app.flags.DEFINE_integer('log_step',
                           default=10,
                           docstring='')
tf.app.flags.DEFINE_integer('save_step',
                           default=100,
                           docstring='')
...
def main(argv):
    ...
    model = cpm_body.CPM_Model(FLAGS.stages, FLAGS.num_of_joints + 1)
...
#                     (FLAGS.input_size, FLAGS.input_size, 1))
            ...
            # Recreate heatmaps
            gt_heatmap_np = cpm_utils.make_gaussian_batch(batch_y_np, FLAGS.heatmap_size, FLAGS.gaussian_radius)
...
#                 image_val = tf.summary.image("predict_fake", demo_stage_heatmaps[0])
#                 summary.value.add(tag='name of var', simple_value=image_val)
