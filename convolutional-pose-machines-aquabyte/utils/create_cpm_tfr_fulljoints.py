...
for person_dir in os.listdir(dataset_dir):
    ...
    for idx, line in enumerate(gt_content):
        ...
        if not line[0].endswith(('jpg', 'png')):
            continue
        cur_img_path = dataset_dir + person_dir + "/" +  line[0]
        print(cur_img_path)
        ...
        if cur_img.shape[0] > cur_img.shape[1]:
            ...
            if SHOW_INFO:
                ...
                for i in range(num_of_joints):
                    cv2.circle(output_image, (int(cur_hand_joints_x[i]), int(cur_hand_joints_y[i])), 8, (0, 255, 0), 4)

                    # Generate joint gaussian map
                    part_heatmap= cpm_utils.gaussian_img(box_size,box_size,cur_hand_joints_x[i],cur_hand_joints_y[i],gaussian_radius)
                ...
                for i in range(num_of_joints):
                    ...
                    #                                               [cur_hand_joints_x[i], cur_hand_joints_y[i]])
                    output_heatmaps[:, :, i]= cpm_utils.gaussian_img(box_size,box_size,cur_hand_joints_x[i],cur_hand_joints_y[i],gaussian_radius)
            ...
            if SHOW_INFO:
                ...
                for i in range(num_of_joints):
                    ...
                    part_heatmap = cpm_utils.gaussian_img(output_image.shape[0], output_image.shape[0], cur_hand_joints_x[i], cur_hand_joints_y[i], gaussian_radius)
                    # part_heatmap = utils.make_gaussian(output_image.shape[0], gaussian_radius,
#                                                        [cur_hand_joints_x[i], cur_hand_joints_y[i]])
                    hmap += part_heatmap * 50
            else:
