...
class CPM_Model(object):
    ...
    def build_model(self, input_image, center_map, batch_size):
        ...
        tf.summary.image('center_map', self.center_map)
        s_counter = 1;
        for s in self.stage_heatmap:
        	string_val = "heatmap_" + str(s_counter)
        	tf.summary.image(string_val + '_0', tf.slice(s, [0, 0, 0, 0], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_1', tf.slice(s, [0, 0, 0, 1], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_2', tf.slice(s, [0, 0, 0, 2], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_3', tf.slice(s, [0, 0, 0, 3], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_4', tf.slice(s, [0, 0, 0, 4], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_5', tf.slice(s, [0, 0, 0, 5], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_6', tf.slice(s, [0, 0, 0, 6], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_7', tf.slice(s, [0, 0, 0, 7], [self.batch_size, 64, 64, 1]))
        	tf.summary.image(string_val + '_8', tf.slice(s, [0, 0, 0, 8], [self.batch_size, 64, 64, 1]))
        	s_counter += 1;
    ...
    def build_loss(self, gt_heatmap, lr, lr_decay_rate, lr_decay_step):
        ...
        tf.summary.image('ground_truth_heatmap_slice_0', tf.slice(self.gt_heatmap, [0, 0, 0, 0], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_1', tf.slice(self.gt_heatmap, [0, 0, 0, 1], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_2', tf.slice(self.gt_heatmap, [0, 0, 0, 2], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_3', tf.slice(self.gt_heatmap, [0, 0, 0, 3], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_4', tf.slice(self.gt_heatmap, [0, 0, 0, 4], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_5', tf.slice(self.gt_heatmap, [0, 0, 0, 5], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_6', tf.slice(self.gt_heatmap, [0, 0, 0, 6], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_7', tf.slice(self.gt_heatmap, [0, 0, 0, 7], [self.batch_size, 64, 64, 1]))
        tf.summary.image('ground_truth_heatmap_slice_8', tf.slice(self.gt_heatmap, [0, 0, 0, 8], [self.batch_size, 64, 64, 1]))
