...
### COCO Metrics
### Based Upon:
###   https://github.com/rafaelpadilla/review_object_detection_metrics/blob/684551117c96c4ec2f5ac78a35dab75ca8135a07/src/evaluators/coco_evaluator.py#L1
###   MIT License: https://github.com/rafaelpadilla/review_object_detection_metrics/blob/684551117c96c4ec2f5ac78a35dab75ca8135a07/LICENSE#L3
### Ported to work with MFT-PG ImgWithBoxes.
### Modified to export some additional metrics.
###
...
def get_coco_summary(groundtruth_bbs, detected_bbs):
    ...
    Parameters
        ----------
            groundtruth_bbs : list
                A list containing objects of type ImgWithBoxes representing the ground-truth bounding boxes.
            detected_bbs : list
                A list containing objects of type ImgWithBoxes representing the detected bounding boxes.
    Returns:
    ...
    def _evaluate_images(iou_threshold, max_dets, area_range):
        # accumulate evaluations on a per-image basis
        _img_evals = defaultdict(lambda: {"scores": [], "matched": [], "NP": []})
        for img_id, class_id in _bbs:
            ev = _evaluate_image(
                _bbs[img_id, class_id]["dt"],
                _bbs[img_id, class_id]["gt"],
            ...
            acc = _img_evals[img_id]
            acc["scores"].append(ev["scores"])
            acc["matched"].append(ev["matched"])
            acc["NP"].append(ev["NP"])

        # now reduce accumulations
        for img_id in _img_evals:
            acc = _img_evals[img_id]
            acc["scores"] = np.concatenate(acc["scores"])
            acc["matched"] = np.concatenate(acc["matched"]).astype(np.bool)
            acc["NP"] = np.sum(acc["NP"])
    ...
    full_iou05_images = _evaluate_images(iou_threshold=0.5, max_dets=100, area_range=(0, np.inf))
    ...
    image_id_to_stats = dict(
                         (r['image_id'],
                            {
                                'Recall1_iou05': (r['TP'] or 0.) / (r['total positives'] or 1),
    ...
    return {
        ...
        "image_id_to_stats": image_id_to_stats,
    }
...
def _get_area(a):
    ...
    x, y, x2, y2 = a.get_x1_y1_x2_y2()#get_absolute_bounding_box(format=BBFormat.XYX2Y2)
    return float(x2 - x) * (y2 - y)


def _jaccard(a, b):
    xa, ya, x2a, y2a = a.get_x1_y1_x2_y2()#.get_absolute_bounding_box(format=BBFormat.XYX2Y2)
    xb, yb, x2b, y2b = b.get_x1_y1_x2_y2()#.get_absolute_bounding_box(format=BBFormat.XYX2Y2)
