...
class sealiceSVMtrain(object):
    ...
    def collect_lice_ORB_train_data(self):
        ...
        np.save('lice_SVM_train_data.npy',lice_SVM_train_data)
    ...
    def collect_nonlice_ORB_train_data(self):
        ...
        np.save('nonlice_SVM_train_data.npy',nonlice_SVM_train_data)
    ...
    def trainSVMmodel(self,train_data,train_labels):
        ...
        C_grid.maxVal=1e10
        C_grid.logStep=2
        Gamma_grid = mdl.getDefaultGridPtr(cv2.ml.SVM_GAMMA)
        Gamma_grid.minVal = 1e-10
        Gamma_grid.maxVal = 1e10
        Gamma_grid.logStep=2
        mdl.trainAuto(train_data, cv2.ml.ROW_SAMPLE, train_labels,10,C_grid,Gamma_grid)
        # Save trained model 
        mdl.save("sealice_detection_ORB_SVM_model_6.yml")
        return mdl
...
def main():
    ...
    model = sealice_svm_trainer.trainSVMmodel(np.array(data, np.float32), np.array(labels, np.int32))
