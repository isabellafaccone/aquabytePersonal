...
let AVAIL_EXECUTORS=$NUM_CORES/5
...
aws emr create-cluster --name "Aloks Spicy Cluster" --release-label emr-5.27.0 --applications Name=Spark \
    ...
    --steps Type=Spark,Name="template matching",ActionOnFailure=TERMINATE_CLUSTER,Args=[--deploy-mode,cluster,--master,yarn,--conf,spark.yarn.submit.waitAppCompletion=false,--num-executors,$NUM_EXECUTORS,--executor-cores,5,--executor-memory,${EXECUTOR_MEM}g,$CODE_LOCATION] \
