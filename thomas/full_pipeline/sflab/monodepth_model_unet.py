...
def jaccard_coef_loss(y_true, y_pred):
    return -tf.log(jaccard_coef(y_true, y_pred)) + tf.nn.sigmoid_cross_entropy_with_logits(labels=y_true,logits=y_pred)


class MonodepthModel(object):
    """monodepth model"""

    def __init__(self, params, mode, left, right, mask, reuse_variables=None, model_index=0):
        self.params = params
        self.mode = mode
        self.left = left
        self.right = right
    ...
    def IOU_(self, y_pred, y_true):
        ...
        Returns:
            float: IOU score
        """
        H, W = y_pred.get_shape().as_list()[1:]
    ...
    def build_vgg(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            udisp4  = self.upsample_nn(iconv4, 2)

            upconv3 = upconv(iconv4,  64, 3, 2) #H/4
            concat3 = tf.concat([upconv3, skip2, udisp4], 3)
            iconv3  = conv(concat3,   64, 3, 1)
            # self.disp3 = self.get_disp(iconv3)
            udisp3  = self.upsample_nn(iconv3, 2)

            upconv2 = upconv(iconv3,  32, 3, 2) #H/2
            concat2 = tf.concat([upconv2, skip1, udisp3], 3)
            iconv2  = conv(concat2,   32, 3, 1)
            # self.disp2 = self.get_disp(iconv2)
            udisp2  = self.upsample_nn(iconv2, 2)

            upconv1 = upconv(iconv2,  16, 3, 2) #H
            concat1 = tf.concat([upconv1, udisp2], 3)
            iconv1  = conv(concat1,   16, 3, 1)
            # self.disp1 = self.get_disp(self.iconv1)
            self.seg = conv(iconv1, 1, 1, 1, activation_fn=tf.nn.sigmoid)
    ...
    def build_resnet50(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            # self.disp1 = self.get_disp(self.iconv1)
            self.seg = conv(self.iconv1, 1, 1, 1, activation_fn=tf.nn.sigmoid)
...
#             self.disp_right_est = [tf.expand_dims(d[:,:,:,1], 3) for d in self.disp_est]
        ...
        with tf.variable_scope('segmentation'):
            self.pred = tf.squeeze(self.seg, [3])
...
#             self.lr_loss = tf.add_n(self.lr_left_loss + self.lr_right_loss)
            ...
            # BINARY CROSS ENTROPY SEG LOSS
            # self.flat_logits = tf.reshape(self.seg1, [1, -1])
            # self.flat_labels = tf.reshape(self.mask, [1, -1])
            # print(flat_logits)            
            # self.seg_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.flat_labels, logits=self.flat_logits)
            
            # TOTAL LOSS
#             self.total_loss = 0*self.image_loss + 0*self.params.disp_gradient_loss_weight * self.disp_gradient_loss + 0*self.params.lr_loss_weight * self.lr_loss + self.seg_loss
            print(self.pred)
            print(self.mask)
            # self.total_loss = -self.IOU_(self.pred, self.mask)
            self.total_loss = jaccard_coef_loss(self.mask, self.pred)
