...
--            vol = vol:transpose(3, 4):transpose(2, 3):clone()
      ...
      if opt.a == 'predict' then
         ...
         print(('Writing /output/%s.bin, %d x %d x %d x %d'):format(fname, vol:size(1), vol:size(2), vol:size(3), vol:size(4)))
         torch.DiskFile(('/output/%s.bin'):format(fname), 'w'):binary():writeFloat(vol:float():storage())
         collectgarbage()
      end
...
if opt.a == 'predict' then
   ...
   print(('Writing /output/disp.bin, %d x %d x %d x %d'):format(disp:size(1), disp:size(2), disp:size(3), disp:size(4)))
   torch.DiskFile('/output/disp.bin', 'w'):binary():writeFloat(disp:float():storage())
   os.exit()
end
